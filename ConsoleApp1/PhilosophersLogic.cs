...
class PhilosophersLogic
{
    ...
    private static readonly object consoleLock = new object(); // Controle para exibir informações no console
    ...
    private static void AtualizarEstado(int filosofoId, string novoEstado)
    {
        ...
        AtualizarInterface?.Invoke(filosofoId, novoEstado);  // Notifica a interface sobre a mudança de estado
    }
    ...
    private static void ExibirEstados()
    {
        lock (consoleLock)
        {
            ...
            for (int i = 0; i < NUM_FILOSOFOS; i++)
            {
                Console.WriteLine($"Filósofo {i}: {estados[i]}");
            }
        }
    }
}
